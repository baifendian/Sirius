...
opentsdb_url = "http://172.24.2.104:9999"
...
class AddCodisProxyApply(threading.Thread):
    #@ brief   初始化类的所有成员
    #@ param[in]  codisinfo   codis信息
    def __init__(self,codisinfo,ZKADDR):
        threading.Thread.__init__(self)
        self.codisinfo=codisinfo
        self.zkinfo=ZKADDR
...
def GenerateCommandLog(productid,commandname,command,commandret):
    global indexline
    configini=open(commnd_dir+('command%s.log')%productid,'a+')
...
def ViewLog(request,productid):
    log=open(commnd_dir+('command%s.log')%productid,'a+')
    codislog=[]
    if log:
        for line in log:
...
#@ brief 添加新的proxy
def AddCodisProxy(request,codis_id):
    httpResponse=HttpResponse()
    codislist=Codis.objects.filter(codis_id=int(codis_id))
    if codislist:
        ...
        return httpResponse
    t1=AddCodisProxyApply(codisinfo,ZKADDR)
    t1.start()
    result='添加proxy申请已提交'
    httpResponse.write(result)
    return httpResponse
...
#@ brief 一键删除codis
#@ param[in] codis_id
def DeleteCodis(request,codis_id):
    httpResponse=HttpResponse()
    codislist=Codis.objects.filter(codis_id=int(codis_id))
    if codislist:
        codisinfo=codislist[0]
...
def ApplyMem(request,method):
    if method=="applymem":
        ...
        t1=NewApply(productid,requiremem,ZKADDR,GetDashHost()[1:4])
        t1.start()
        mess='2'
        return render_to_response('applymem.html', locals(), context_instance=RequestContext(request))
    else:
...
#@ brief    网页内为现有codis服务扩容的接口
#@ param[in] request 
def AddMem(request,method):
    if method=="addmem":
        productid=""
        requiremem=-1
        if request.method=="POST":     
            ...
            productid = request.GET.get('product_id',"")
            requiremem = long(request.GET.get('need_memory',""))
        ac_logger.debug('ADDMEM-------------------')
        t1=AlterMem(productid,requiremem)
        t1.start()
        mess='2'
        return render_to_response('addmem.html', locals(), context_instance=RequestContext(request))
    else:
        codis_list=Codis.objects.all()
        return render_to_response('addmem.html', locals(), context_instance=RequestContext(request))


#@ brief   网页内添加主机的接口
#@ param[in] request 
def AddHost(request,method):
    if method=="addhost":
        hostid=-1
        hostip=""
        hostuser=""
        ...
        if request.method=="POST":
            ...
            hostpasswd=request.POST.get('host_pass',"")
            memtotal=int(request.POST.get('memory_total',""))
            codishome=request.POST.get('codis_home',"")
        else:
            hostip=request.GET.get('host_ip',"")
            hostuser=request.GET.get('host_user',"")
            hostpasswd=request.GET.get('host_pass',"")
            memtotal=int(request.GET.get('memory_total',""))
            codishome=request.GET.get('codis_home',"")
        host_list = Host.objects.filter(host_ip=hostip)
        if host_list:
            mess='1'
            return render_to_response('addhost.html', locals(), context_instance=RequestContext(request))
...
#@ brief  删除编号为hostid的主机
#@ param[in] request
def DelHost(request):
    #mysqlctl=MySQLCtl()
    hostid=-1
    if request.method=="POST":
        hostid=int(request.POST.get('host_id',""))
    else:
        hostid=int(request.GET.get('host_id',""))
    #mysqlctl.DeleteHost(hostid)
    try:
        Host.objects.get(pk=hostid)
        Host.delete()
    except Exception:
        ac_logger.debug('id错误，查找失败2')    
...
class ServerInfo(APIView):
    '''
    '''
    def get(self, request, codis_id, format=None):
        ...
        try:
            ...
            for group in result_list:
                for server in group['servers']:
                    ...
                    server['key_info'] = redis_info['db0']
...
class ProxyInfo(APIView):
    ...
    '''
    def get(self, request, codis_id, format=None):
        proxy_list = Proxy.objects.filter(codis_id=int(codis_id)).order_by('proxy_id')
        proxy_l = []
        for proxy in proxy_list:
            proxy_dict = {}
            proxy_dict['proxy_id'] = proxy.proxy_id
            proxy_dict['proxy_ip'] = proxy.proxy_ip
            proxy_dict['proxy_port'] = proxy.proxy_port
            proxy_dict['http_ip'] = proxy.http_ip
            proxy_dict['http_port'] = proxy.http_port
    ...
    def delete(self, request, proxy_id, format=None):
        proxy = Proxy.objects.get(proxy_id=proxy_id)     
        result = deletetheproxy(proxy)
        return packageResponse(result)
...
class ApplyNewCodis(APIView):
    '''
        申请codis
    '''
    def post(self, request, format=None):
        product_id = request.Get.get('request_id')
        memory_size = request.Get.get('memory_size')
        t1=NewApply(product_id, memory_size, ZKADDR,GetDashHost()[1:4])
        t1.start()
        result = {
            "code":"0",
            "msg":"OK"
